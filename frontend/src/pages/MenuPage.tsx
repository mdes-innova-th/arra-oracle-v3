...
function MenuRows({ items, emptyText = 'No menu items returned from /api/menu.' }: { items: MenuItem[]; emptyText?: string }) {
  ...
  return (
    <div className="glass overflow-hidden rounded-2xl border border-[oklch(1_0_0/0.05)] bg-[oklch(0.20_0.02_265/0.25)] backdrop-blur-md">
      <div className="hidden overflow-x-auto md:block">
        <table className="min-w-full divide-y divide-white/10 text-left text-sm">
          <thead className="glass bg-[oklch(0.20_0.02_265/0.25)] backdrop-blur-md text-xs font-semibold uppercase tracking-[0.16em] text-text-muted">
            <tr>
      ...
      <ul className="grid gap-2 p-3 md:hidden" aria-label="Menu items">
        {items.map((item) => (
          <li key={menuKey(item)} className="glass rounded-xl border border-[oklch(1_0_0/0.05)] bg-[oklch(0.20_0.02_265/0.25)] backdrop-blur-md p-3">
...
function MenuFiltersCard({
  ...
  return (
    <section className="glass mb-5 rounded-2xl border border-[oklch(1_0_0/0.05)] bg-[oklch(0.20_0.02_265/0.25)] backdrop-blur-md p-4" aria-label="Menu filters">
...
export function MenuPage({ items: initialItems = [], loading, client = apiClient, initialSearch }: MenuPageProps) {
  ...
  return (
    <section className="glass rounded-3xl border border-[oklch(1_0_0/0.08)] bg-[oklch(0.16_0.02_265/0.35)] shadow-[0_8px_32px_oklch(0_0_0/0.4)] backdrop-blur-xl p-5 sm:p-6" aria-labelledby="menu-page-title">
