...
function FeedRows({ items }: { items: SearchResult[] }) {
  ...
  return (
    <div className="grid gap-3" aria-label="DB-backed document feed">
      {items.map((item) => (
        <article key={item.id} className="glass min-w-0 rounded-2xl border border-[oklch(1_0_0/0.05)] bg-[oklch(0.20_0.02_265/0.25)] backdrop-blur-md p-4">
...
export function FeedPage({ load = fetchDocumentFeed }: { load?: FeedLoader }) {
  ...
  return (
    <section className="glass w-full min-w-0 rounded-3xl border border-[oklch(1_0_0/0.08)] bg-[oklch(0.16_0.02_265/0.35)] shadow-[0_8px_32px_oklch(0_0_0/0.4)] backdrop-blur-xl p-5 sm:p-6" aria-labelledby="feed-title">
