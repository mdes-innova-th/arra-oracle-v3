...
<script type="module">
let plugins=${JSON.stringify(plugins)};let plugin=${JSON.stringify(id)};const apiBase=${JSON.stringify(apiBase)};const canvas=document.querySelector('canvas');const ctx=canvas.getContext('2d');
...
function normalizeRegistry(value){const list=Array.isArray(value?.plugins)?value.plugins:null;return list?.map((item)=>({id:item.id,label:item.label,kind:item.kind,href:item.standalonePath||item.href||(item.id==='map'||item.id==='planets'?'/'+item.id:'/?plugin='+item.id)})).filter((item)=>item.id&&item.label&&item.kind)}
...
async function loadRegistry(){useCachedRegistry(readLocal());const cached=await readIdb();if(cached)useCachedRegistry(cached);try{const latest=await (await fetch('/api/canvas/registry')).json();useRegistry(latest,'registry updated');await cacheRegistry(latest)}catch{await cacheRegistry({plugins})}}
