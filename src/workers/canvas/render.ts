...
let plugin=${JSON.stringify(id)};
let pluginData={documents:[]};
const apiBase=${JSON.stringify(apiBase)};
const canvasHost=${JSON.stringify(CANVAS_HOST)};
const canvas=document.querySelector('canvas');
const ctx=canvas.getContext('2d');
const CACHE_KEY='oracle.canvas.registry.v1';const DB_NAME='oracle-canvas-cache';
const status=document.getElementById('status');const error=document.getElementById('error');const picker=document.getElementById('plugin-picker');
const title=document.getElementById('canvas-title');const subtitle=document.getElementById('canvas-subtitle');const nav=document.querySelector('nav[aria-label="Canvas plugins"]');
let t=0;let frame=0;let activeCleanup=null;
...
function pluginKind(item){const value=String(item?.kind||item?.renderer||'').toLowerCase();if(value==='react'||value==='three')return value;return item?.id==='map'||item?.id==='planets'?'react':'three'}
function pluginHref(item){return item.standalonePath||item.href||(item.id==='map'||item.id==='planets'?'/'+item.id:'/?plugin='+item.id)}
function normalizeRegistry(value){const list=Array.isArray(value?.plugins)?value.plugins:null;return list?.map((item)=>({id:item.id,label:item.label||item.name||item.id,kind:pluginKind(item),apiPath:item.apiPath,href:pluginHref(item)})).filter((item)=>item.id&&item.label&&item.kind)}
function renderPickerOptions(){const current=plugin;picker.replaceChildren(...plugins.map((item)=>{const option=document.createElement('option');option.value=item.id;option.selected=item.id===current;option.textContent=item.label+' · '+item.kind;return option}));nav.replaceChildren(...plugins.map((item)=>{const link=document.createElement('a');link.href=pluginHref(item);link.dataset.pluginLink=item.id;link.textContent=item.label;link.toggleAttribute('aria-current',item.id===current);link.addEventListener('click',(event)=>{event.preventDefault();setPlugin(item.id)});return link}))}
function useRegistry(value,label){const next=normalizeRegistry(value);if(!next?.length)return false;plugins=next;renderPickerOptions();if(!plugins.some((item)=>item.id===plugin))plugin=plugins[0].id;setPlugin(plugin,true);status.textContent=label;return true}
...
function disposeActivePlugin(){if(activeCleanup)activeCleanup();activeCleanup=null;if(frame)cancelAnimationFrame(frame);frame=0;ctx.clearRect(0,0,innerWidth,innerHeight)}
function renderLoop(paint){const tick=()=>{t+=.012;ctx.clearRect(0,0,innerWidth,innerHeight);ctx.fillStyle='#020617';ctx.fillRect(0,0,innerWidth,innerHeight);paint();frame=requestAnimationFrame(tick)};frame=requestAnimationFrame(tick);return()=>{if(frame)cancelAnimationFrame(frame)}}
function mountThreePlugin(meta){document.body.dataset.hotSwapMode='three dispose/mount';return renderLoop(()=>paintPlugin(meta))}
function renderReactPlugin(meta){document.body.dataset.hotSwapMode='react swap';return renderLoop(()=>paintPlugin(meta))}
function mountPlugin(meta){return meta.kind==='react'?renderReactPlugin(meta):mountThreePlugin(meta)}
function setPlugin(next,replace=false){const meta=plugins.find((item)=>item.id===next);if(!meta)return;disposeActivePlugin();plugin=meta.id;document.body.dataset.plugin=plugin;document.body.dataset.kind=meta.kind;picker.value=plugin;title.textContent='Oracle '+meta.label+' Canvas';subtitle.textContent=canvasHost+' · plugin='+meta.id+' · '+meta.kind;document.querySelectorAll('[data-plugin-link]').forEach((a)=>a.toggleAttribute('aria-current',a.dataset.pluginLink===plugin));loadPluginData(meta);activeCleanup=mountPlugin(meta);if(!replace)history.pushState({plugin},'',pluginHref(meta))}
picker.addEventListener('change',()=>setPlugin(picker.value));
addEventListener('popstate',()=>{const path=location.pathname.slice(1);const query=new URLSearchParams(location.search).get('plugin');setPlugin(query||path||'wave',true)});
...
function paintPlugin(meta){const n=Math.min(dataCount(),90);if(meta.id==='cube'||meta.id==='torus'){ctx.strokeStyle=meta.id==='cube'?'#5eead4':'#c4b5fd';ctx.lineWidth=4;ctx.strokeRect(innerWidth/2-90,innerHeight/2-90,180,180);ctx.strokeRect(innerWidth/2-45+Math.sin(t)*40,innerHeight/2-45,90,90)}else if(meta.id==='galaxy'){for(let i=0;i<120;i++)star(i)}else if(meta.id==='solar'||meta.id==='planets'){for(let i=0;i<(n||9);i++){const a=t+i*.72;dot(innerWidth/2+Math.cos(a)*(70+i*26),innerHeight/2+Math.sin(a)*(35+i*14),6+i*.7,i%2?'#67e8f9':'#c4b5fd')}}else if(meta.id==='map'||meta.id==='map3d'||meta.id==='graph3d'){for(let i=0;i<(n||45);i++)dot((i*97)%innerWidth,(Math.sin(t+i)*120+innerHeight/2),3,'#2dd4bf');ctx.strokeStyle='#22d3ee66';ctx.strokeRect(innerWidth*.16,innerHeight*.24,innerWidth*.68,innerHeight*.52)}else{ctx.strokeStyle='#5eead4';ctx.lineWidth=3;ctx.beginPath();for(let x=0;x<innerWidth;x+=8){const y=innerHeight/2+Math.sin(x*.018+t*4)*90;ctx[x?'lineTo':'moveTo'](x,y)}ctx.stroke()}}
renderPickerOptions();setPlugin(plugin,true);loadRegistry();
