...
export function noEvidence(sources: AskSource[]): boolean {
...
export async function synthesize(question: string, sources: AskSource[], client?: AskClient): Promise<AskSynthesis> {
  const noEvidenceFlag = noEvidence(sources);
  if (noEvidenceFlag) return { answer: 'No evidence found in indexed oracle documents.', citations: [], noEvidence: true, mode: 'extractive' };
  if (!client) return extractiveAnswer(sources, noEvidenceFlag);
  let raw: unknown;
  try { raw = await client(promptFor(question, sources)); } catch { return extractiveAnswer(sources, noEvidenceFlag); }
  const parsed = parseLlm(raw);
  if (!parsed) return extractiveAnswer(sources, noEvidenceFlag);
  return { answer: parsed.answer, citations: validCitations(parsed.citations, sources), noEvidence: Boolean(parsed.noEvidence), mode: 'llm' };
}
...
function promptFor(question: string, sources: AskSource[]): AskPrompt {
  return {
    instruction: [
      ...
      'If sources do not support an answer, return noEvidence=true and say so.',
...
function extractiveAnswer(sources: AskSource[], noEvidenceFlag: boolean): AskSynthesis {
  const cited = sources.slice(0, 3);
  const answer = cited.map((source) => `[${source.index}] ${source.excerpt}`).join('\n');
  return { answer, citations: cited.map((source) => source.index), noEvidence: noEvidenceFlag, mode: 'extractive' };
}
