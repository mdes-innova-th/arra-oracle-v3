...
export async function mineCommand(args: string[]): Promise<number> {
  ...
  try {
    options = parseMineArgs(args);
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
    console.error(mineHelp());
    return 1;
  }
  if (options.help) {
    console.log(mineHelp());
    return 0;
  }
  if (!options.dir) {
    console.error('mine requires a directory');
    console.error(mineHelp());
    return 1;
  }
  const print = (r: { stored: number; scanned: number; skipped: number; project: string }) => {
    const docs = `document${r.stored === 1 ? '' : 's'}`;
    const files = `file${r.scanned === 1 ? '' : 's'}`;
    console.log(`Mined ${r.stored} ${docs} from ${r.scanned} ${files} (${r.skipped} skipped) into project "${r.project}".`);
  };
  try {
    if (options.watch) {
      ...
      print(await mineFolder(options as { dir: string; dbPath?: string; dryRun?: boolean }));
    }
    return 0;
  } catch (error) {
    console.error(error instanceof Error ? error.message : String(error));
